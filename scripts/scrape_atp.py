...
REPO_BASE = "https://raw.githubusercontent.com/Aneeshers/tennis-sackmann-archive/main/atp"
...
# Fail closed when an upstream source is unavailable or incomplete. The weekly
# workflow must never replace a healthy production database with a tiny partial
# scrape, which is what happened when the original Sackmann repository vanished.
MIN_PLAYER_BIOS = 10_000
MIN_MATCH_YEARS = 50
MIN_RANKED_PLAYERS = 1_000
MIN_OUTPUT_PLAYERS = 3_000
...
def main():
    ...
    if len(players_csv) < MIN_PLAYER_BIOS:
        raise RuntimeError(
            f"Player biography source is incomplete: received {len(players_csv)} rows "
            f"(expected at least {MIN_PLAYER_BIOS}). Existing output was preserved."
        )
    ...
    if loaded_match_years < MIN_MATCH_YEARS:
        raise RuntimeError(
            f"Historical match source is incomplete: loaded {loaded_match_years} seasons "
            f"(expected at least {MIN_MATCH_YEARS}). Existing output was preserved."
        )

    print(f"  Processed matches for {len(stats)} players across {loaded_match_years} seasons")
    ...
    if len(peak_rankings) < MIN_RANKED_PLAYERS:
        raise RuntimeError(
            f"Ranking source is incomplete: loaded {len(peak_rankings)} players "
            f"(expected at least {MIN_RANKED_PLAYERS}). Existing output was preserved."
        )
    ...
    if len(all_players) < MIN_OUTPUT_PLAYERS:
        raise RuntimeError(
            f"Generated database is incomplete: built {len(all_players)} players "
            f"(expected at least {MIN_OUTPUT_PLAYERS}). Existing output was preserved."
        )
...
def generate_js(player_data):
    ...
    lines.append("// Source: Jeff Sackmann tennis_atp data via Aneeshers/tennis-sackmann-archive (CC BY-NC-SA 4.0)")
